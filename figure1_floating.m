...
load('floating_catmullData.mat')
optDegDiff = 12;

%% FLOATING OBJECTS

numCells = numCells6;
recordAccumulationCells = recordAccumulationCells6;
...
for cell = 1:1:numCells
    ...
    % have one distribution for MT25 : 6p is recordAccumulationCells6 2
    meanMT25raw = recordAccumulationCells(:,1,cell,2); % may either have 9 or 11 values
    meanMT25raw(rangeObjOri) = flipud(meanMT25raw(rangeObjOri));

    % have one distribution for MT-25 : 6n is recordAccumulationCells6 1
    meanMTn25raw = recordAccumulationCells(:,1,cell,1); % may either have 9 or 11 values
    meanMTn25raw(rangeObjOri) = flipud(meanMTn25raw(rangeObjOri));
    ...
    title('+25º Monkey Tilt, Boxcar Smoothed Data, Floating Objects','fontname','lato','FontSize',15)
